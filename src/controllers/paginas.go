...
// CarregarMural carrega a página de mural com todas as publicacoes na rede
func CarregarMural(w http.ResponseWriter, r *http.Request) {
	url := fmt.Sprintf("%s/todasPublicacoes", config.APIURL)
	response, erro := requisicoes.FazerRequisicaoComAutenticacao(r, http.MethodGet, url, nil)
	if erro != nil {
		respostas.JSON(w, http.StatusInternalServerError, respostas.ErroAPI{Erro: erro.Error()})
		return
	...
	utils.ExecutarTemplate(w, "mural.html", struct {
		Publicacoes []modelos.Publicacao
		UsuarioID   uint64
	}{
		Publicacoes: publicacoes,
